<template>
  <div class="profile">
    ...
    <table
      class="profile-stats"
      v-if="Array.isArray(profile.stats)"
    ...
    <div class="profile-section" v-if="profile?.footnotes" v-show="isFullView">
      <p>&#10145; <em>{{ profile.footnotes.footnotes }}</em></p>
    </div>
    <div class="profile-section" v-if="profile?.specialRules" v-show="isFullView">
    ...
    <div class="profile-section" v-if="profile?.abilities" v-show="isFullView">
    ...
    <div class="profile-section" v-if="profile?.invulnerable" v-show="isFullView">
      <h2 class="profile-subtitle">{{ i18n.INVULNERABLE }} <strong>{{ profile.invulnerable }}</strong></h2>
    </div>
    <div class="profile-section" v-if="profile?.damaged" v-show="isFullView">
      <h2 class="profile-subtitle">{{ i18n.DAMAGED }}</h2>
      <p>{{ profile.damaged }}</p>
    </div>
    <div class="profile-section" v-if="profile?.factionKeywords" v-show="isFullView">
      <h2 class="profile-subtitle">{{ i18n.FACTION_KEYWORDS }}</h2>
      <!-- TODO: añadir links de cada keyword -->
      <p>{{ profile.factionKeywords.join(', ') }}</p>
    </div>
    <div class="profile-section" v-if="profile?.keywords" v-show="isFullView">
...
import i18n from '@/api/en.i18n.json'
